...
export default function SignUp({ onSuccess, onNavigateToLogin }: SignUpProps) {
  ...
  const isDeviceActiveBySchedule = (schedule: any, controlState: string, deviceData?: any, skipIndividualLimitCheck?: boolean): boolean => {
    ...
    if (deviceData && !skipIndividualLimitCheck) {
      ...
      if (powerLimit > 0 && totalMonthlyEnergy >= powerLimit) {
        console.log(`SignUp: Schedule check - Device monthly power limit exceeded:`, {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(combinedLimitRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        console.log('SignUp: Real-time update - all departments combined limit data:', allDepartmentsData)
        ...
        console.log('SignUp: No combined limit settings found')
      ...
      console.error('SignUp: Error listening to combined limit info:', error)
      setAllDepartmentCombinedLimits({})
      setCombinedLimitInfo({
        enabled: false,
        selectedOutlets: [],
  ...
  useEffect(() => {
            ...
            if (deviceData.schedule && 
              ...
              if (isInCombinedGroup && deviceDepartmentLimit) {
                ...
                if (totalMonthlyEnergy >= combinedLimitkW) {
                  ...
                  limitsExceeded = true
                  newControlState = 'off'
                  console.log(`🔒 SignUp: FORCING ${outletKey} OFF - MONTHLY LIMIT EXCEEDED for department ${deviceDepartmentLimit.department} - SKIPPING SCHEDULE CHECK`)
                  ...
                  console.log(`🔒 SignUp: Enforced device_control='off' for ${outletKey} due to monthly limit exceeded in department ${deviceDepartmentLimit.department}`)
                }
                ...
                if (totalMonthlyEnergy >= combinedLimitkW) {
                  limitsExceeded = true
                  newControlState = 'off'
                  console.log(`🔒 SignUp: FORCING ${outletKey} OFF - OLD COMBINED MONTHLY LIMIT EXCEEDED - SKIPPING SCHEDULE CHECK`)
                }
                ...
                if (powerLimit > 0) {
                  ...
                  if (totalMonthlyEnergy >= powerLimit) {
                    ...
                    console.log(`🔒 SignUp: FORCING ${outletKey} OFF - INDIVIDUAL MONTHLY LIMIT EXCEEDED - SKIPPING SCHEDULE CHECK`)
                  }
                }
              } else {
                // Device is in combined group but detection failed - log warning and skip individual check
                console.log(`⚠️ SignUp: Device ${outletKey} detection issue - isInCombinedGroup=${isInCombinedGroup}, deviceDepartmentLimit=${!!deviceDepartmentLimit}, isInOldCombinedGroup=${isInOldCombinedGroup} - SKIPPING INDIVIDUAL LIMIT CHECK`)
              }
              ...
              if (!limitsExceeded) {
                ...
                console.log(`✅ SignUp: Limits OK for ${outletKey} - Current state: ${currentControlState}, Schedule says: ${shouldBeActive ? 'ON' : 'OFF'}, New state: ${newControlState}`)
              }
              
              console.log(`SignUp: Final status determination for ${outletKey}:`, {
          ...
          for (const [outletKey, outletData] of Object.entries(devicesData)) {
            ...
            if (!isInCombinedGroup) {
              ...
              if (powerLimit > 0) {
                ...
                if (totalMonthlyEnergy >= powerLimit) {
                  console.log(`SignUp: POWER LIMIT EXCEEDED - Turning OFF ${outletKey} (${(totalMonthlyEnergy * 1000).toFixed(3)}W >= ${(powerLimit * 1000)}W)`)
          ...
          if (totalMonthlyEnergy >= combinedLimitkW) {
            ...
            for (const outletKey of combinedLimitInfo.selectedOutlets) {
              ...
              try {
                ...
                const currentMainStatus = deviceData?.relay_control?.main_status || 'ON'
                if (currentMainStatus === 'ON') {
                  console.log(`⚠️ SignUp: Skipping ${outletKey} - main_status is ON (bypass mode/override active)`)
                  skippedCount++
                  continue
                }
                ...
                console.log(`✅ SignUp: TURNED OFF ${outletKey} (${firebaseKey}) due to monthly limit`)
                successCount++
              } catch (error) {
                console.error(`❌ SignUp: FAILED to turn off ${outletKey}:`, error)
                failCount++
              }
            }
            ...
            if (currentDeviceControl !== 'off') {
              ...
              console.log(`🔒 SignUp: Set combined_limit_settings/device_control='off' to prevent re-activation`)
            }
            
            console.log(`🔒 SignUp: MONTHLY LIMIT ENFORCEMENT COMPLETE: ${successCount} turned off, ${skippedCount} skipped (bypass mode), ${failCount} failed`)
          } else {
            console.log('✅ SignUp: Monthly limit not exceeded - devices can remain active')
            ...
            if (currentDeviceControl !== 'on' || currentEnforcementReason !== '') {
              ...
              console.log(`✅ SignUp: Set combined_limit_settings/device_control='on' (limit not exceeded)`)
            }
          }
        }
      } catch (error) {
    ...
    // This prevents devices from being incorrectly turned off when navigating to SignUp.tsx
